using LocalDecoders
using ArgParse, JLD2, Combinatorics, Statistics, Random
...
can compute:
    * time dependence of logical fidelity (probability of logical error after time proportional to system size)
    * relaxation times
    * statistical properties and correlation functions in long-time steady states
    * spacetime history of error-correcting dynamics for fixed initial configuration and noise distribution 
    * exhaustive search to find problematic low-weight errors 
...
function main(;
    ...
    mode: what the code calculates. one of the following:
        ...
        * "hist": runs a single spacetime history of the dynamics; used for visualization
        * "noise_search": searches over noise configurations satisfying some failure criterion
    ...
    """
    if nps === nothing
        nps = mode == "trel" ? (l > 1 ? 7 : 9) : (mode ∈ ["Ft" "offline"] ? 10 : (mode ∈ ["hist" "noise_search"] ? 1 : (mode == "stats" ? 11 : 1)))
    end
    if logdist === nothing
    ...
    ps,pc,samps,thermal_periods,periods = parameter_repository(model,mode,l,nps,logdist,bias,gadgetnoise,measurementnoise,test;pmin_override=pmin,pmax_override=pmax)
    fout = joinpath(DATADIR, "$(model)_l$(l)_$(mode)_$(gate)"*(mode == "trel" ? "_$(periods)pers" : "")*"$(out_adj).jld2")
    if mode == "trel"
        fout = joinpath(DATADIR, "$(model)_l$(l)_$(mode)_$(gate)$(out_adj).jld2")
    end
    if mode == "stats"
        fout = joinpath(DATADIR, "$(model)_l$(l)_$(mode)_$(gate)$(out_adj).jld2")
    end
    ...
    fin = joinpath(DATADIR, "$(model)_l$(l)_gates_$(gate)$(adj).jld2")
    if isfile(fin)
    ...
    elseif mode == "offline"
        ...
        for (pind,p) in enumerate(ps)
            ...
            for samp in 1:samps
                ...
                # decode
                data["offline_error_rate"][pind] += detect_logical_failure(state) ? 1/samps : 0
    ...
    ### run a single spacetime history (for visualization purposes) ###
    elseif mode == "hist"
        println("running single spacetime history...")
        p = ps[1]
        initial_state = bitrand(length(codeword))
        if model ∈ ["rep" "rep_5bit" "k2" "twod_rep"]
            history, _ = master_gate_applier(model,copy(initial_state),[],gates,periods,[p*0],bias,gadgetnoise_dict,squarenoise,measurementnoise,true)
            data["history"] = history
            println("final ⟨spin⟩ = ", mean(history[end,:]))
        elseif model == "tc"
            init_errs = falses(Lx,Ly,2)
            if Lx ≥ 9 
                init_errs[4,4:6,2] .= true # vertical 1-link error 
                init_errs[4:6,7,1] .= true # horizontal 1-link error (combines with above to form 1-corner error)
            end 
            init_synds = errs_to_synds(init_errs)
            noise_hist = falses(periods*depth+1,Lx,Ly,4)
            # apply iid noise 
            # for t in 1:periods*depth+1, i in 1:Lx, j in 1:Ly, o in 1:2
            #     if rand() < p noise_hist[t,i,j,o] = true end
            # end
            err_hist, synd_hist = tc_gate_applier!(copy(init_synds),copy(init_errs),gates,periods,noise_hist,gadgetnoise_dict,squarenoise,measurementnoise,true)
            data["err_hist"] = err_hist; data["synd_hist"] = synd_hist; data["noise_hist"] = noise_hist
            println("final anyon count = ", sum(synd_hist[end,:,:]))
        end

    ### search over noise histories satisfying some criterion ###
    elseif mode == "noise_search"
        println("running noise search...")
        ...
        if model ∈ ["rep" "rep_5bit" "k2" "twod_rep"]
            ...
            while ~failed && thistry < maxtries
                history, _ = master_gate_applier(model,copy(initial_state),[],gates,1,[p],bias,false,false,measurementnoise,true)
...
function parse_args()
    s = ArgParseSettings(description="Monte Carlo simulation of self-correcting circuit dynamics")
    @add_arg_table! s begin
        "--mode"
            help = "simulation mode: Ft, trel, stats, hist, noise_search, offline"
            default = "offline"
        "--model"
            help = "code model: rep, rep_5bit, k2, tc, twod_rep"
            default = "tc"
        "--gate"
            help = "gate type: Z, R, decoder, Y, id, etc."
            default = "R"
        "--test"
            help = "use reduced samples/periods for testing"
            action = :store_true
        "--level", "-l"
            help = "level (log_n of system size)"
            arg_type = Int
            default = 1
        "--squarenoise"
            help = "apply plaquette errors instead of link errors"
            action = :store_true
        "--measurementnoise"
            help = "noise from syndrome measurement failures only"
            action = :store_true
        "--bias"
            help = "noise bias"
            arg_type = Float64
            default = 0.0
        "--gadgetnoise"
            help = "gadget-based noise model"
            action = :store_true
        "--nps"
            help = "number of noise values to sample"
            arg_type = Int
            default = -1
        "--logdist"
            help = "use log-spaced noise values"
            action = :store_true
        "--pmin"
            help = "minimum noise value (overrides default from parameter_repository)"
            arg_type = Float64
            default = NaN
        "--pmax"
            help = "maximum noise value (overrides default from parameter_repository)"
            arg_type = Float64
            default = NaN
        "--calc-corrs"
            help = "calculate 2-point correlation functions"
            action = :store_true
        "--measure-corrs-multiple"
            help = "measure correlations every N steps"
            arg_type = Int
            default = 500
        "--calc-dw-stats"
            help = "calculate domain wall size statistics"
            action = :store_true
        "--rollstate"
            help = "random cyclic spatial shift between periods"
            action = :store_true
        "--adj"
            help = "distinguishing info for input file (gates)"
            default = ""
        "--out-adj"
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    main(;
        ...
        test = args["test"],
        l = args["level"],
        squarenoise = args["squarenoise"],
        measurementnoise = args["measurementnoise"],
        bias = args["bias"],
        gadgetnoise = args["gadgetnoise"],
        nps = args["nps"] == -1 ? nothing : args["nps"],
        logdist = args["logdist"] ? true : nothing,
        calc_corrs = args["calc-corrs"],
        measure_corrs_multiple = args["measure-corrs-multiple"],
        calc_dw_stats = args["calc-dw-stats"],
        pmin = isnan(args["pmin"]) ? nothing : args["pmin"],
        pmax = isnan(args["pmax"]) ? nothing : args["pmax"],
        rollstate = args["rollstate"],
        adj = args["adj"],
        out_adj = args["out-adj"],
    )
end
