using LocalDecoders
using ArgParse, JLD2, FileIO, Random, Statistics, LinearAlgebra
...
function main(;
    direct_search = false,
    clean_input_search = true,
    dirty_input_search = false,
    maxlevel = nothing)

    if maxlevel === nothing
        maxlevel = direct_search ? 4 : 10
    end
...
function parse_args()
    s = ArgParseSettings(description="Nilpotence testing for toric code automaton gadgets")
    @add_arg_table! s begin
        "--direct-search"
            help = "direct search through spacetime volume of level-l gadgets"
            action = :store_true
        "--clean-input-search"
            help = "recursive search with clean inputs (default if no flags given)"
            action = :store_true
        "--dirty-input-search"
            help = "recursive search with arbitrary input syndromes"
            action = :store_true
        "--maxlevel"
            help = "maximum number of levels to search"
            arg_type = Int
            default = -1
    end
...
if abspath(PROGRAM_FILE) == @__FILE__
    args = parse_args()
    # default to clean_input_search if no search flags given
    any_search = args["direct-search"] || args["clean-input-search"] || args["dirty-input-search"]
    main(;
        direct_search = args["direct-search"],
        clean_input_search = any_search ? args["clean-input-search"] : true,
        dirty_input_search = args["dirty-input-search"],
        maxlevel = args["maxlevel"] == -1 ? nothing : args["maxlevel"],
    )
end
